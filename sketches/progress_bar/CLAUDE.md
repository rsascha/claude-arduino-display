...
Fortschrittsanzeige mit fünf Segmenten, groß und zentriert. Alle 3 Sekunden kommt eines
dazu; nach dem fünften ist Schluss, und ein Druck auf **EXIT** startet den Durchlauf neu —
auch mitten drin. Kein WLAN, keine `secrets.h`.
...
Der eigentliche Zweck ist ein anderer als die Anzeige: Es ist der Anwendungsfall, für den
`EPD_PartUpdate()` gedacht ist. Der Vergleich in `../ha_umschalten` hatte gezeigt, dass
Partial für einen Vollbildwechsel nichts taugt. Hier ändert sich pro Schritt nur ein
Segment — genau dafür ist der Modus da.
...
In `../bedienleiste` fiel das auf, weil dort Vollrefresh und Teilrefresh abwechseln: Nach
dem Wischen stand das Ruhebild auf dem Panel, in `0x26` aber noch der Rest des
...
Dort schreibt deshalb `merkeAltesBild()` den Bildpuffer nach `0x26`/`0xA6`, mit derselben
Adressrechnung wie `EPD_Display()`.
...
Vor einem **vollen oder schnellen** Neuaufbau gehört `EPD_Clear_R26A6H()` dagegen gar
nicht; dort richtet er Schaden an. Die Kreuztabelle dazu steht in
`../bedienleiste/CLAUDE.md`.
...
Eine Textzeile braucht `size` Pixel Höhe, nicht weniger — in `../ha_verlauf` hatten sich
zwei Zeilen um 6 px überlappt, weil das zu knapp gerechnet war. Vertikale Abstände vorher
...
- **`EPD_DrawRectangle()` zieht im gefüllten Modus nur bis `Yend-1`** — die letzte Zeile
  fehlt. Die Segmente füllt der Sketch deshalb selbst über `safePixel()` (`fillRect()`).
- **Ein 1-px-Umriss ist auf dem Panel kaum zu sehen.** Der Balkenrahmen besteht aus drei
  ineinandergelegten Rechtecken (`frameRect()`).
...
- `../bedienleiste/CLAUDE.md` — Partial mit Tastenbedienung, Refresh-Kreuztabelle und
  `merkeAltesBild()`
- `../ha_umschalten/CLAUDE.md` — der Vergleich der drei Refresh-Modi
- `../../CLAUDE.md` → *Harte Fakten* — die Regel zu `0x26` in Kurzfassung
